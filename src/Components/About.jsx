      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {services.map((s, idx) => (
            ...
            >
              <div className={`w-20 h-20 mb-6 rounded-2xl flex items-center justify-center text-white shadow-lg bg-gradient-to-br ${s.color} group-hover:scale-105 transition-transform duration-400`}>
                {s.icon}
              </div>
              <h3 className="text-xl font-semibold mb-2 group-hover:text-amber-700">{s.title}</h3>
              <p className="text-gray-600">{s.text}</p>

              <div className="absolute top-0 right-0 w-20 h-20 bg-gradient-to-bl from-amber-100/50 to-transparent rounded-bl-full opacity-0 group-hover:opacity-100 transition-opacity duration-400"></div>
            </div>
          ))}
        </div>
      </div>
